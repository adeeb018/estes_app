...
class _SettingsPageState extends State<SettingsPage> {
  ...
  late int selectedContainer;// = imageNames.indexOf(storeController.currentBackground);
  int selectedFont = -1;

  @override
  void initState() {
    borderColor = Colors.red;
    selectedContainer = imageNames.indexOf(storeController.currentBackground);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBarWidget(
        /////////////appbar widget is called and a function parameter is passed so that back functionality can be implemented
        onpressed: () {
          Get.back();
        },
      ),
   ...
   */
  Container _styleButton(String font, String styleText) {
    return Container(
  ...
  Widget _themeButton(String imageName, int themeValue, int index) {
    return GestureDetector(
      onTap: () {
        ...
        storeController.currentBackground =
            imageName; //////////////on clicking any themeButton in grid view the getXcontroller is updated with current theme,
        selectedContainer =
            index; ////////////////////////////////// background, and a selection variable is also changed to indicate it is selected.
        setState(() {});
      },
      child: Container(
        decoration: BoxDecoration(
          image: imageName != ''
              ? (index + 1 != imageNames.length
                  ? DecorationImage(
                      ...
                      /////////// for first page and last page there will be no images present
                      fit: BoxFit.fill)
                  : null)
              : null,
          ...
          border: selectedContainer == index
              ...
              : (index + 1 == imageNames.length
                  ? Border.all(
                      /////////////////selected container border should be pink and the final container border should be black
