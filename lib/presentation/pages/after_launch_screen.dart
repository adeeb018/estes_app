...
class _AfterLaunchState extends State<AfterLaunch> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBarWidget(onpressed: () async {
        ...
        storeController.bluetoothScreen
            .buttonAction([0xA0, 0x01, 0x00, 0xA1]).then((_) =>
                storeController.bluetoothScreen.buttonAction([
                  0xA0,
                  0x02,
                  0x00,
                  0xA2
                ])); /////here both lights of device has been turned off

        Future ft = Future(() {});
        ft = ft.then((_) {
          return Future.delayed(const Duration(milliseconds: 200), () {
            ////////////////////////here on pressing back button we navigate to 5 th currentView of homePage.
      ...
      bottomNavigationBar: (currentState == 3 || currentState == 5)
          ? const SizedBox()
          : SizedBox(
              height: 60,
              child: TextButton(
  ...
  CorousalText bottomNavigationcorousalText() {
    switch (currentState) {
      case 1:
        return CorousalText(text: 'Allow permissions', color: Colors.white);
      case 2:
        return CorousalText(text: 'Troubleshoot Guide', color: Colors.white);
      case 4:
        return CorousalText(text: 'Help & Guide', color: Colors.white);
      default:
        return CorousalText(text: 'Error', color: Colors.white);
    }
  }
  ...
  Widget pageTextInfo() {
    switch (currentState) {
      ...
      case 2:
        return CorousalText(
          text: 'We are facing some issue\nplease wait while we figure it out',
          color: Colors.white,
        );
      case 3:
        return CorousalText(
      ...
      case 5:
        return Padding(
          ...
          child: CorousalText(
            text: 'You have successfully launched\nthe rocket ',
            color: Colors.white,
          ),
        );
      default:
